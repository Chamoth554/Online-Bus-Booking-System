...
if (isset($_POST['buy'])) {
    ...
    if (!isset($_POST['bus_id']) || !isset($_POST['jdate']) || !isset($_POST['fare'])) {
        echo '<div class="alert alert-danger"><strong>Error: </strong>Required fields missing</div>';
    } else {
        $stmt = $conn->prepare("INSERT INTO tickets (passenger_id, bus_id, jdate, seats, fare) VALUES (?, ?, ?, NULL, ?)");
        $stmt->bind_param("iiss", $_SESSION['user']['id'], $_POST['bus_id'], $_POST['jdate'], $_POST['fare']);

        if ($stmt->execute()) {
            echo '<div class="alert alert-success">Purchase <strong>Success</strong>ful!<a class="text-right" href="print.php?ticket=' . $conn->insert_id . '"><button class="btn btn-info">Print</button></a></div>';
        } else {
            echo '<div class="alert alert-danger"><strong>Error: </strong>' . $stmt->error . '</div>';
        }
        ...
        <div class="row content">
            ...
            <div class="col-sm-1">
                <form method="post" action="">
                    ...
                    <input type="hidden" name="jdate" value="' . $_GET['jdate'] . '">
                    <input type="hidden" name="fare" value="' . $row['fare'] . '">
                    <button type="submit" name="buy" class="btn btn-primary btn-sm">Buy Ticket</button>
                </form>
            </div>
        </div>';
    }
